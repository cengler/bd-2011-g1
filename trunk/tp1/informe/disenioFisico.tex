...
\\
Basándonos en el Modelo Relacional confeccionado se armaron scripts en SQL Server 2005 para la creación de la base de datos BDtp1Grupo1 y sus objetos necesarios para cumplir con los requerimientos del enunciado.
En la entrega digital del trabajo práctico se incluyen los siguientes scripts:
\begin{itemize}
...
\begin{center}
\fbox{\includegraphics[scale=0.50]{img/DiagramaBaseFisica1.png}}
\end{center}

\begin{center}
\fbox{\includegraphics[scale=0.50]{img/DiagramaBaseFisica2.png}}
\end{center}

\subsection{Scripts}
En esta sección explicaremos con más detalle los scripts mencionados con anterioridad, incluyendo los motivos de su creación en función de las necesidades del presente trabajo práctico.

\subsubsection{Creación de la Base}
...
Para esto se utilizó el script \textbf{BD-CreacionTablas}.\\
...
Se crearon las siguientes tablas:
	\begin{itemize}
		...
		\item\textbf{Contingencia}: Representa los posibles eventos que pueda surgir un viaje qe ya llegó a destino.
...
\item \textbf{Ciudad}: Representa una ciudad para la cual se defiriniran N registros en la tabla direccion.
...
\item \textbf{Vehiculo}: Esta tabla representa todos los vehiculos, tanto en uso como en reparación.
...
\item \textbf{Participa}: Realciona las tablas Clima, Estacion y Ruta. Para cada Ruta y cada Estacion debe haber un registro con el Clima de dicha Ruta en dicha Estacion.
\item \textbf{Estacion}: Representa las 4 estaciones del año.
\item \textbf{Clima}: Representa los posibles climas.
\end{itemize}

Se decidió crear una sola tabla para representar todos los vehículos (tabla vehiculo), tanto los que estén en reparación como los que estén en uso, ya que la única diferencia entre ellos es un campo de tipo fecha.
En este caso, como la herencia de vehiculo a vehiculoEnUso y vehiculoEnReparacion es dijunta se utiliza un campo, cuyo nombre es enUso, que admite 2 valores (0 si el vehiculo está en reparación y 1 si el vehiculo esta en uso).
Luego, a traves de una constraint en la tabla, restringimos que si un vehiculo tiene un 0 en el campo enUso entonces debe tener fecha de ingreso a reparacion (campo fechaIngresoReparacion) y esta fecha debe ser posterior a la fecha de alta del vehiculo (fechaAlta). 


\subsubsection{Agregado de restricciones}
Para el agregado de constraints se empleó el script \textbf{BD-AgregadoConstraits}.\\
...
Las constraints que se incluyeron en las tablas con las sigueintes:
\begin{itemize}
...
Se agregaron los siguientes triggers para validar la insersión y actualización de datos en viajePlanificado y la actualizacion de vehiculos. 
Estos triggers cumplen con el requerimiento de \textit{'Implementación de alguna restricción adicional que surja del diseño'}.\\
Las siguientes son algunas de las restricciones que se podrían hacer en la estructura de base de datos mediante triggers.\\
Hay otras, por ejemplo que no se hicieron como verificar que cada vez que se asocia un chofer a un viaje planificado a traves de la tabla Conduce no haya 3 choferes ya asociados a ese viaje, que no fueron implementadas.\\
Con el siguiente script se valida que el viaje que se esté insertando se esté asociando a un vehiculo que esté en uso y no en reparación.
...
Con el siguiente script se valida que el viaje que se esté actualizando no haya sido realizado.
\begin{verbatim}
...
Si es así, no permitimos la actualización.*/
...
Con el siguiente script se valida que el vehiculo que se este modificando, pasando a estar en reparacion, no tenga asociado un viaje planificado que todavia no se haya realizado.
\begin{verbatim}
/*
...
Si es así, no permitimos la actualizacion.*/
...
\subsubsection{Creación de Vistas}
...
\begin{itemize}
\item \textit {\textbf{`'Los recorridos para los cuales se usaron todas las rutas posibles registradas para ese recorrido, para viajes realizados el año pasado y recorridos asociados a más de una ruta"}}.\\
\\
Se implementó la vista \textbf{recorridosConTodasRutasUsadasAnioAnterior} que cumple con el requerimiento de listar `'Los recorridos para los cuales se usaron todas las rutas posibles registradas para ese recorrido, para viajes realizados el año pasado y recorridos asociados a más de una ruta".
\\
...
/*
	Se crea la vista cantRutasXRecorrido devuelve el codigo de recorrido y 
...
/*
	Se crea la vista cantRutasRecorridasAnioAnterior devuelve el nombre del recorrido, el codigo 
...
\item \textit{\textbf{`'El promedio de viajes realizados por vehículo por año y el estado en que éste se encuentra"}}.\\
\\
Además, se implementó la vista \textbf{promedioEstadoXVehiculo} para cumplir con el requerimiento de listar `'El promedio de viajes realizados por vehículo por año y el estado en que éste se encuentra".\\
Esta vista utiliza la vista auxiliar \textbf{cantViajesXVehiculoXAnio} que devuelve la cantidad de viajes de cada vehiculo por cada año de actividad. \\
Debajo se adjuntan las implementaciones de cada una de las vistas:
\\
\begin{verbatim}
...
\item \textit{\textbf{Los choferes que han utilizado todos los vehículos de menos de dos años de antigüedad, en viajes del último semestre}}.\\
\\
Por ultimo, se implementó la vista \textbf{chofer6MVehiculo2A} que devuelve `'Los choferes que han utilizado todos los vehículos de menos de dos años de antigüedad, en viajes del último semestre."
...
\item \textbf{viajesChoferesUltimos6Meses}: filtra los viajes realizados por los choferes quedándose con los que fueron realizados en los últimos 6 meses.   
\end{itemize}
...
Como también, se pueden obtener resultados para las vistas `'recorridosConTodasRutasUsadasAnioAnterior", `'promedioEstadoXVehiculo" y `'chofer6MVehiculo2A".
