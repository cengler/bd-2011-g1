...
\item \textbf{Ciudad}: Representa una ciudad para la cual se definirán N registros en la tabla direccion.
...
Luego, a través de un constraint en la tabla, restringimos que si un vehiculo tiene un 0 en el campo enUso entonces debe tener fecha de ingreso a reparacion (campo fechaIngresoReparacion) y esta fecha debe ser posterior a la fecha de alta del vehiculo (fechaAlta). 
...
Se agregaron los siguientes triggers para validar la inserción y actualización de datos en viajePlanificado y la actualizacion de vehiculos. 
...
Con el siguiente script se valida que el viaje que se esté insertando esté asociado a un vehiculo que en uso.
...
Con el siguiente script se valida que el vehiculo que se esté modificando, pasando a estar en reparación, no tenga asociado un viaje planificado que todavía no se haya realizado.
\begin{verbatim}
/*
