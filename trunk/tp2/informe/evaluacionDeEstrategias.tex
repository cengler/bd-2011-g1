...
\begin{framed}
\begin{verbatim}
-- TRAZA REQUEST TUPLAS 2 PAGINAS DISTINTAS 2 
Request([A_index, 0])
Release([A_index, 0])
Request([A_index, 1])
Release([A_index, 1])
...
-- TRAZA REQUEST TUPLAS 4 PAGINAS DISTINTAS 2 
Request([A_index, 0])
Release([A_index, 0])
Request([A_index, 1])
Release([A_index, 1])
...
-- TRAZA REQUEST TUPLAS 4 PAGINAS DISTINTAS 10 
Request([A_index, 0])
Release([A_index, 0])
Request([A_index, 1])
Release([A_index, 1])
...
\textbf{PC: cantidad de páginas} y \textbf{RC: Cantidad de request}
\\\\
Se puede ver en este gráfico, que el $hit-rate$ depende más del caso testeado que del algoritmo de reemplazo utilizado. Se nota que MRU es un poco menos performante que FIFO o LRU. Esto se debe a que en general es más esperable en este tipo de pruebas que se repita una de las últimas páginas referenciadas.
