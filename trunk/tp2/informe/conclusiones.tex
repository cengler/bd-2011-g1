...
\subsection{\isu}
  En este algoritmo no se puede predecir qué estrategia de reemplazo de frames es conveniente, pero a diferencia del \isc se puede observar que es posible que se resoliciten bloques ya cacheados anteriormente. Es decir, la coincidencia de que una página ya esté en memoria depende de la traza y de la estrategia de reeamplazo que se esté utilizando.\newline
De los casos evaluados anteriormente ( en la sección Evaluación de Estrategias/Index Scan Unclustered), los cuales se tomaron al azar, se observó que MRU es el que da un poco peor de los tres. 
Tanto en FIFO como en LRU se liberaban las páginas de los índices, cosa que no ocurre con MRU. Es decir, que este útimo no las desalojaba.\\
Cuando la cantidad de páginas dedicadas a devolver tuplas es pequeña, en comparación a las páginas del índice, se convierte en una desventaja para MRU.
\subsection{\bnlj}
...
La idea de las pruebas realizadas fue variar los bloques asignados a R de tal forma que sobren bloques de memoria y de esta manera ver que ocurre cuando se tiene más de un bloque libre de memoria para cachear bloques de S. Esto no sucede en el algoritmo tradicional de \bnlj ya que en éste solo se le dedica un bloque de memoria a S. \newline
Como resultado de las pruebas obtuvimos que, en esta variación de \bnlj, se pueden llegar a tener cacheados, dependiendo de las estrategias de reemplazo, algunos bloques de S que son solicitados nuevamente y obtener así un hit rate mayor a cero. \newline
Se puede concluir que MRU es la estrategia de reemplazo que mejor trabaja en \bnlj, tal como se ve en las Figuras 2 y 3 de la sección 4.4.
