...
\item \textbf{BD-InsercionDatos}: Inserta datos iníciales para testear la integridad de estructura de la base de datos.
\end{itemize}
...
Se crearon las siguientes tablas:
	\begin{itemize}
		...
		\item\textbf{Contingencia}: Representa los posibles eventos que puedan surgir en un viaje que ya llegó a destino.
...
\item \textbf{Ciudad}: Representa una ciudad para la cual se definirán N registros en la tabla dirección.
...
\item \textbf{Estado}: Refleja los posibles estados de un vehículo.
\item \textbf{Vehiculo}: Esta tabla representa todos los vehículos, tanto en uso como en reparación.
...
\item \textbf{Participa}: Relaciona las tablas Clima, Estación y Ruta. Para cada Ruta y cada Estación debe haber un registro con el Clima de dicha Ruta en dicha Estación.
...
En este caso, como la herencia de vehículo a vehiculoEnUso y vehiculoEnReparacion es disjunta se utiliza un campo, cuyo nombre es enUso, que admite 2 valores (0 si el vehículo está en reparación y 1 si el vehículo esta en uso).
Luego, a través de un constraint en la tabla, restringimos que si un vehículo tiene un 0 en el campo enUso entonces debe tener fecha de ingreso a reparación (campo fechaIngresoReparacion) y esta fecha debe ser posterior a la fecha de alta del vehículo (fechaAlta). 
...
Para todos los campos de tipo texto por simplicidad se decidió que su longitud sea de 30 caracteres.   

\subsubsection{Agregado de restricciones}
Para el agregado de constraints se empleó el script \textbf{BD-AgregadoConstraits}.\\
Se agregaron constraints en las tablas de licencia, en la de vehículo y la tabla recorrido que restringe el ingreso de datos comparando datos en campos de la misma tabla.\\
...
Se agregaron los siguientes triggers para validar la inserción y actualización de datos en viajePlanificado y la actualización de vehículos. 
Estos triggers cumplen con el requerimiento de \textit{ ``Implementación de alguna restricción adicional que surja del diseño''}.\\
Hay otras restricciones (ej: que al asociar un chofer a un viaje planificado a través de la tabla Conduce no haya 3 choferes ya asociados a ese viaje) que no fueron implementadas.\\
Con el siguiente script se valida que el viaje que se esté insertando esté asociado a un vehículo que esté en uso.
...
Con el siguiente script se valida que el vehículo que se esté modificando, pasando a estar en reparación, no tenga asociado un viaje planificado que todavía no se haya realizado.
\begin{verbatim}
/*
...
BEGIN
   ...
   está asignado a un viaje planificado.'', 16, 1)
   ROLLBACK TRANSACTION
END')
\end{verbatim}
...
/*
	Se crea la vista cantRutasXRecorrido devuelve el código de recorrido y 
...
/*
	Se crea la vista cantRutasRecorridasAnioAnterior devuelve el nombre del recorrido, el código 
...
/*
	...
	manejó cada chofer en los ultimos 6 meses.
*/
